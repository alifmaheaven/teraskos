...
export default {
    login({commit}, user){
        return new Promise((resolve, reject) => {
          var is_production = process.env.VUE_APP_URL_API_IS_PRODUCTION == 'true' ? true : false;
          commit('auth_request')
          axios({ url: process.env.VUE_APP_URL_API_USER + '/loginhcis', data: user, method: 'POST' })
              .then(resp => {
                  ...
                  if (user.id_role != 3) {
                    ...
                    var err = {response:{statusText:'Hanya admin dan moderator yang boleh masuk!'}}
                    console.log(err);
    ...
    getTokenTelkom({commit}){
      return new Promise((resolve, reject) => {
        ...
        axiosIndependent({ 
                method: 'get',
                url:  process.env.VUE_APP_URL_GATEWAY_BASEURL_TELKOM+'rest/pub/apigateway/jwt/getJsonWebToken?app_id=095428f5-5de7-44e3-bcc1-1e3c23dfffff',
    ...
    getnotification({commit}, token){
      return new Promise((resolve, reject) => {
        commit('response_request')
        axios({ url: process.env.VUE_APP_URL_API_USER + '/getusernotification', method: 'GET' })
            .then(resp => {
              commit('response_success')
              resolve(resp)
            })
    ...
    getUser({commit}){
      ...
      return new Promise((resolve, reject) => {
        commit('response_request')
        axios({ url: process.env.VUE_APP_URL_API_USER + '/getuser', method: 'GET' })
            .then(resp => {
                commit('response_success')
    ...
    accessLog({commit},data){
      return new Promise((resolve, reject) => {
        commit('response_request')
        axios({ url: process.env.VUE_APP_URL_API_USER + '/accesslog', data: data, method: 'POST' })
            .then(resp => {
                commit('response_success')
                resolve(resp)
            })
